...
def main(argv):
    ...
    TEST, OUTPUT, HELP, path = False, "", False, '.'
    ...
    if HELP:
            ...
            print("The training and dev-data are assumed to live in ")
            print("Options:")
            print(" -t         evaluate on test instead of dev")
            print(" -o [path]  create output files with guesses (and don't just evaluate)")
    ...
    for lang in sorted(list({re.sub('\.train.*$','',d) for d in os.listdir(path) if '.train' in d})):
        trainpath = path+lang+".train"
        ftunepath = path+lang+".ftune"
        evalpath = path + "_".join(lang.split("_")[:-1]) + ".dev"
        print(OUTPUT)
        outpath = OUTPUT+lang + ".dev"
        if TEST:
            evalpath = path + "_".join(lang.split("_")[:-1]) + ".gold"
            outpath = OUTPUT+lang + ".test"
        outfile = open(outpath, "w")
        evallines = [line.strip() for line in open(evalpath, "r") if line != '\n']
        print()
        print("language", lang)
        print("training", trainpath, ftunepath)
        print("evaluation", evalpath)
        print("output", outpath)

        allprules, allsrules = {}, {}
        if not os.path.isfile(trainpath):
            print("COULD NOT FIND", trainpath)
            continue
        lines = [line.strip() for line in open(trainpath, "r") if line != '\n']
        lines.extend([line.strip() for line in open(ftunepath, "r") if line != '\n'])
